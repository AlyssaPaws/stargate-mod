    ...
    public class WorldComp_StargateAddresses : WorldComponent
    {
        ...
        public void CleanupAddresses()
        {
            foreach (PlanetTile pT in new List<PlanetTile>(AddressList))
            {
                ...
                if (site != null)
                {
                    tagsPresent = (site.MainSitePartDef.tags.Contains("StargateMod_StargateSite") ||
                                   site.MainSitePartDef.tags.Contains("StargateMod_StargateOrbitalSite")); 
                }
                if (sgMap == null || (!sgMap.HasMap && (site == null || !tagsPresent) && !(sgMap is WorldObject_PermSGSite)))
                {
                    RemoveAddress(pT);
                }
            }
